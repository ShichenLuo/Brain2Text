...
scripts/                One-off dataset and lexicon preparation scripts
...
The original research scripts remain at the repository root during this cleanup so
existing notebook experiments continue to open without breaking. New work should
use the package layout and keep generated files under `data/processed/` or
`artifacts/`.

## Pipeline

1. Place source recordings or annotated corpus files in `data/raw/`.
2. Clean corpus text with `process_corpus.py`.
3. Convert cleaned sentences to phonemes with `create_phone.py`.
